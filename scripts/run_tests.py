...
class TemplateWithDefaultKey(Template):
    ...
    If the table is being tested, then it should appear in the translation map, pointing the table to a different location.
    If the table is not being tested, it will fall back to the right table, prefixed with the right dataset.
    """

    def __init__(self, template, base_dataset: str):
    ...
    def substitute(self, *args, **kwds):
        try:
            ...
            key = str(err.args[0])
            kwds[key] = f"{self.base_dataset}.{key}"
...
def get_parser() -> ArgumentParser:
    ...
    arg_parser.add_argument('--base-dataset', required=True,
                            help='The base BigQuery Dataset, to fall back on if table is '
                                 'not found in the translation file', dest='base_dataset')
...
def run_test_file(bigquery_client: bigquery.Client, base_dataset: str, translations: dict[str:str],
    ...
    query_template = TemplateWithDefaultKey(sql_content, base_dataset)
...
def run_tests(bigquery_client: bigquery.Client, base_dataset: str, translations: dict[str:str], test_file_path: str):
    if not os.path.exists(test_file_path):
        raise Exception(f"{test_file_path} does not exists")
    if os.path.isfile(test_file_path):
        test_result = run_test_file(bigquery_client, base_dataset, translations, test_file_path)
        return test_result
    assert os.path.isdir(test_file_path)
    results = {}
    for file in os.listdir(test_file_path):
        results.update(run_test_file(bigquery_client, base_dataset, translations, os.path.join(test_file_path, file)))
    return results


def run(translation_file: str, base_dataset: str, test_file_path: str) -> int:
    """
    Main entry point
    """
    translations = read_json_as_dict(translation_file)
    bigquery_client = create_bigquery_client()
    test_results = run_tests(bigquery_client, base_dataset, translations, test_file_path)
...
def main():
    ...
    exit_code = run(translation_file=args.translation_file, base_dataset=args.base_dataset,
                    test_file_path=args.TEST_FILE_OR_DIR_PATH)
    sys.exit(exit_code)
